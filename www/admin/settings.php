...
// Вспомогательная функция для обновления настроек через PDO
    ...
    try {
        ...
        if (isset($_POST['color_scheme'])) {
            ...
            if ($color_scheme === 'custom' && isset($_POST['custom_colors'])) {
                ...
                log_event("Пользовательские цветовые значения обновлены.");
            ...
            <div class="card-header">Цветовая схема</div>
             <div class="card-body">
                <form action="settings.php" method="post">
                    <?php
                    $schemes = [
                        'default' => 'По умолчанию (темно-синяя)', 'light' => 'Светлая (голубая)',
                        'success' => 'Зеленая', 'danger' => 'Красная', 'warning' => 'Желтая', 'custom' => 'Пользовательская'
                    ];
                    ?>
                    <div class="form-group">
                    ...
                    <div id="custom_scheme_options" style="<?php echo ($current_scheme !== 'custom') ? 'display: none;' : ''; ?>">
                        ...
                        <div class="form-row">
                            ...
                            <div class="form-group col-md-6">
                                <label for="navbar_link_color">Цвет ссылок в навигации</label>
